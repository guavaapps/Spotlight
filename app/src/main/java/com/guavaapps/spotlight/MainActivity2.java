...
//                    mViewModel.getRecc ();
                    ...
                    AppRepo.getInstance ()
                            .getCurrentUser (mSpotifyService, MainActivity2.this, new AppRepo.ResultListener () {
                                @Override
                                public void onUser (UserWrapper userWrapper) {
                                    ...
                                    Handler.createAsync (Looper.getMainLooper ())
                                            .post (() -> {
                                                ...
                                                MongoClient mongoClient = new MongoClient(MainActivity2.this);
                                                mongoClient.login(userWrapper.user.id);

                                                DLSTMPModel model = new DLSTMPModel(new File(""));

                                                TrackLocalDataSource local = new TrackLocalDataSource(userWrapper.user.id);
                                                TrackRemoteDataSource remote = new TrackRemoteDataSource(
                                                        userWrapper.user.id,
                                                        mongoClient,
                                                        model
                                                );
                                                ...
                                                trackRepository.getNext(new float[][] {{0, 1f}});
    ...
    public void auth () {
        ...
        Handler.createAsync (Looper.getMainLooper ()).postDelayed (
                () -> {
                    SpotifyAppRemote.connect (MainActivity2.this, connectionParams, new Connector.ConnectionListener () {
                        @Override
                        ...
                        public void onFailure (Throwable error) {
                            Log.d (TAG, "SpotifyAppRemote failed asRealmObject connect - " + error.getMessage () + " cause=" + error.getCause ());
                        }
                    });
                },
                5000
